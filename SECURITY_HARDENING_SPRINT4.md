...
- [ ] 🟠 **Prerequisite (Workstream 3.5):** migrate the ~25 consumer endpoints in
      `app.py` that still filter by `fields.user_id` to `tenant_id`. Then drop the
      deprecated `fields.user_id`. *Do this before FORCE RLS or the policies fight
      the app's access model.*
- [ ] 🟠 **Add per-tenant RLS policies + `FORCE ROW LEVEL SECURITY`** on the
      tenant-scoped tables (`tenants`, `tenant_members`, `growers`, `fields`,
      `daily_logs`, `harvest_records`, `yield_projections`, `grower_contracts`,
      `input_disbursements`, `deliveries`, `farm_tasks`). Policies must key off the
      authenticated user's tenant membership. This constrains *even the backend role*
      — the compliance-grade isolation a bank/lender reviewer expects.
      - Note: today the 15 RLS-enabled tables show `rls_enabled_no_policy` (INFO) —
        expected and safe (deny-by-default) until these policies land.
      - Requires deciding how the backend authenticates to Postgres post-FORCE
        (e.g. set `request.jwt.claims` / use the authenticated role per request,
        or keep a service path with explicit policies).
